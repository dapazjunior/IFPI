...
async function initApp() {
    try {
        ...
        // Inicializar componentes se autenticado
        ...
        showError('Erro ao inicializar aplicação. Tente recarregar a página.');
    }
}
...
async function initComponents() {
    try {
        ...
        // Inicializar sidebar
        if (typeof sidebarComponent !== 'undefined' && sidebarComponent.init) {
...
function setupProtectedFeatures() {
    // Logout button já é tratado pelo HeaderComponent
}
...
function showError(message) {
    // Usar o componente de alerta se disponível
    if (typeof alertComponent !== 'undefined') {
        alertComponent.error(message);
    } else {
        // Fallback para alerta nativo
        alert(message);
    }
}
